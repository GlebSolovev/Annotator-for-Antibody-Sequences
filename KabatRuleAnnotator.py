#KabatRuleAnnotator 18.01.2019
#count scorel for each rsd and choose max

db_special_mode = True
#'db_special_mode' - special mode 
#for obtainig germline database
input_mode = 'n'
output_mode = ''
print_mode = 'sir'
commentaries = True
...
file_name_inp = 'ObDB Germlines.txt'
file_name_out = 'Annotated Germlines DB: Kabat'
...
def output(res,seq,checklist,domain,info):
    if(db_special_mode):
        print()
    to_print.append('')
    if(len(res) == 0):
...
def scorel(index,data,seq,lastRes):
    ...
    score = 0
    #stpd scoring, SUPER (definitely,not^^) SENSIBLE to any shift(insertions & deletions)
    if(len(pat_x) != 0):
...
def KabatSR5_L(seq,domain,info=[]):
    ...
    res.pop(0)
    output(res,seq,checklist,domain,info)
    #to_print.append('\nSEE:')
    #scorel(res[3][0],'L2_end',seq,res[2][0])
    
def KabatSR5_H(seq,info=[]):
    ...
    for i in range(0,len(script)):
        ...
        if(pos[3] >= 65):
            ...
            comment('{0} undefined (max {1}%)'.format(script[i],pos[3]))
    res.pop(0)
    output(res,seq,checklist,'H',info)
    #to_print.append('\nSEE:')
    #scorel(res[3][0],'L2_end',seq,res[2][0])    

def KabatSR5(seq,domain,info=[]):
    if('H' in domain):
        KabatSR5_H(seq,info)
    elif('k' in domain or 'K' in domain):
        KabatSR5_L(seq,'kappa',info)
    elif('l' in domain): 
        KabatSR5_L(seq,'lambda',info)
    elif('L' in domain):
        comment('\nL-annotation')
...
def main():
    """if(db_special_mode):
        ...
        for i in range (len(input_data)):
            segm = input_data[i]
            if(segm == 'H' or segm == 'k' or segm == 'l'):
                germs = input_data[i+1].split(';')
                for gene in germs:
                    geneInfo = gene.split()
                    info = [geneInfo[k] for k in range(1,len(geneInfo))]
                    info.insert(0,currentSpecies)
                    KabatSR5(geneInfo[0],segm,info)
                i = i+1
            else:
                currentSpecies = segm
            return"""
